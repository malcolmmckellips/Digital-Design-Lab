 ...
 *  machine to transmit a string of data across the LEUART bus. As of Lab 7
 *  there are also functions to support receiving data over LEUART. There are
...
void leuart_open(LEUART_TypeDef *leuart, LEUART_OPEN_STRUCT *leuart_settings){
	...
	//Added in Lab 7 ----------------------------------------------------------------------------------------------------------
	//Format start frame and sig frame if enabled
	if (leuart_settings->startframe_en){
		leuart->STARTFRAME = leuart_settings->startframe;
		while (leuart->SYNCBUSY); //added
	}
	if (leuart_settings->sigframe_en){
		leuart->SIGFRAME = leuart_settings->sigframe;
		while (leuart->SYNCBUSY); //added
	}

	//initialize the rx sfubrx and rxblock enable if enabled
	if (leuart_settings->rxblocken){
		leuart->CMD |= LEUART_CMD_RXBLOCKEN; //Enable rx block
		while (leuart->SYNCBUSY); //added
	}
	if (leuart_settings->sfubrx){
		leuart->CTRL |= LEUART_CTRL_SFUBRX; //Enable sfubrx (start frame unblock on rx)
		while (leuart->SYNCBUSY); //added
	}

	//initialize the rx sfubrx, rxblock enable, and the rx state machine if rx is enabled
	if (leuart_settings->rx_en){
		rx_payload.leuart = leuart;
		rx_payload.rx_state = RX_IDLE;
		sleep_block_mode(LEUART_RX_EM); //Block the sleep mode necessary to enable rx (EM2)
	}
	//End of Added in Lab 7 -----------------------------------------------------------------------------------------------------

	//Configure and enable interrupts
	//Initialize and clear the desired interrupts;
	leuart->IEN = 0;
	leuart->IFC =  LEUART_IFC_TXC | LEUART_IFC_SIGF | LEUART_IFC_STARTF; //modified lab 7 (converted to IFC bits. See last lab to check old way.)

	//Enable the desired interrupts for the tx state machine
	...
	//Enable interrupts for the rx state machine (added in lab 7)
	//Added in Lab 7 ----------------------------------------------------------------------------------------------------------
...
/***************************************************************************//**
 * @brief Function for starting the LEUART module for transmission
 * @details
...
/***************************************************************************//**
 * @brief Function for handling STARTF interrupt
 * @details
 *   Will enter an EFM assert if we receive a STARTF interrupt in any state
 *   except for idle or receive, because all others are invalid states. If we are in the idle state
 *   and we receive a STARTF character, we will initialize our string index to 0 and say that
 *   a receive is in process. We will change state to receive. If we are already in receive
 *   and we receive another start frame, start the transmission over (by setting the string
 *   index to 0).
 *
 *******************************************************************************/
static void leuart_startf(void){
	switch(rx_payload.rx_state){
		case RX_INIT:
			EFM_ASSERT(false);
			break;
		case RX_IDLE:
			//If we are in the correct state, reset the index and say we have started receiving
			rx_payload.index = 0;
			leuart0_rx_busy = true;
			rx_payload.rx_state = RX_RECEIVE;
			break;
		case RX_RECEIVE:
			//If we get a start frame in the middle of another receive,
...
/***************************************************************************//**
 * @brief Function for handling RXDATAV interrupt
 * @details
 * 		EFM_ASSERT unless we are in the receive state or finish state because all others are invalid
 * 		states to receive the RXDATAV interrupt. If we are in the receive state,
 * 		get the current receive data and put it in the receive
 * 		payload buffer at the current index. Then, increment the index. If we get this
 * 		interrupt while in the finishing state, we have received the sigframe. Add the sig frame
 * 		to the receive string and append a null character at the end of our string. Set rx_busy
 * 		to false, schedule an rx_done_evt and return to the idle state after re-setting the rx block.
 *
 *******************************************************************************/
static void leuart_rxdatav(void){
	switch(rx_payload.rx_state){
		...
		case RX_IDLE:
			...
			break;
		case RX_RECEIVE:
			rx_payload.rx_buffer[rx_payload.index] = rx_payload.leuart->RXDATA;
			rx_payload.index ++;
			rx_payload.rx_state = RX_RECEIVE;
			break;
		case RX_FINISH:
			rx_payload.rx_buffer[rx_payload.index] = rx_payload.leuart->RXDATA;
			rx_payload.index++;
			rx_payload.rx_buffer[rx_payload.index] = '\0'; //Add the null character to the end of the string (CHECK MIGHT NOT NEED)
			rx_payload.leuart->CMD |= LEUART_CMD_RXBLOCKEN; //re enable rx block
			leuart0_rx_busy = false;
...
/***************************************************************************//**
 * @brief Function for handling SIGF interrupt
 * @details
 *	EFM_ASSERT if we are in an invalid state. Disregard a sig frame in the idle state
 *	because we should be blocking data in this state. If we are in the receive state,
 *	then a sigf signals we are done with a transmission so move to finish state.
 *******************************************************************************/
 ...
 * 	a tx_done_evt. Will also route the rx interrupts to their necessary handling functions.
 *******************************************************************************/

void LEUART0_IRQHandler(void){
	uint32_t int_flag = LEUART0->IF & LEUART0->IEN; //int flag is all of the bits of interrupts that have occurred and our enabled
	...
	//Added Lab 7 (RXDATAV last to set order of precedence for interrupts)
	if (int_flag & LEUART_IEN_STARTF){
		leuart_startf();
	}
	if (int_flag & LEUART_IEN_SIGF){
			leuart_sigf();
	}
	if (int_flag & LEUART_IEN_RXDATAV){
		leuart_rxdatav();
	}
 ...
 * @param[in] leuart
 * 	The address of the leuart peripheral to use.
 *
 *******************************************************************************/
 ...
 *	 Test 3: Test that data is being blocked before the start frame is received
...
void leuart_rx_test(void){
	...
	while (rx_payload.leuart->SYNCBUSY);
	...
	//Transmit 'a'
	__disable_irq(); //Disable interrupts for the system level tests, so we do not enter our ISR and begin the TX or RX state machine
	...
	EFM_ASSERT(strlen(rx_payload.rx_buffer) == strlen(rx_result_str));
	for (int i = 0; i < strlen(rx_payload.rx_buffer); i++){
		EFM_ASSERT(rx_payload.rx_buffer[i] == rx_result_str[i]);
	}

	//Test 5: Test A second string transmission/receive to ensure that the state machine allows multiple receptions off of a signal initialization
	// 		  This test will also verify that blocking has been re-enabled after a transmission has completed
	EFM_ASSERT(rx_payload.leuart->STATUS & LEUART_STATUS_RXBLOCK);
	char rx_test_str2[100] = "attempt2#SUCCESS!nonsense";
	char rx_result_str2[100] = "#SUCCESS!";
	leuart_start(rx_payload.leuart,rx_test_str2,strlen(rx_test_str2));
	while(leuart0_tx_busy);
	while(leuart0_rx_busy);

	EFM_ASSERT(strlen(rx_payload.rx_buffer) == strlen(rx_result_str2));
	for (int i = 0; i < strlen(rx_payload.rx_buffer); i ++){
		EFM_ASSERT(rx_payload.rx_buffer[i] == rx_result_str2[i]);
	}
...
/***************************************************************************//**
 * @brief
 *   Global function to export private leuart receive buffer state machine variable
 *
 * @details
 * 	The state machine variable rx_buffer will be copied into the destination specified by
 * 	an input argument. This provides for proper modularity and encapsulation
 *
 * @param[in] destination
 *   The string destination to copy the rx_buffer to.

 ******************************************************************************/
void get_leuart_rx_buffer(char *destination){
	strcpy(destination,rx_payload.rx_buffer);
}
